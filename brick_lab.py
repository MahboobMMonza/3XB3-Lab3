...
between 1 and n, it is guaranteed to break at n, so we only need to consider levels 0 to n - 1 for non-breaking levels.

Case 1:
If m >= floor(log_2(n - 1)) + 1, then it is possible to binary search without issues. floor(log_2(n - 1)) + 1 runs on
the machine are required to know what k* is (as stated in the lab instructions). Note that log_2(0) = 0 in this
calculation. The only exception would be that no testing would be required if there is only 1 level.
...
Case 3:
When there are not enough bricks to do a binary search, "try" every level and see which one gives the lowest worst-case.
At each level w, if the brick breaks, check w - 1 levels with one less brick. If the brick doesn't break, then check
i - w levels with the same number of bricks. Of these two options, choose the one with the highest total as that is the
worst-case and add 1 to it (to represent the force test at level w). Then, among all i levels, choose the one with the
lowest worst-case if the first brick was tested at that level to determine the lowest number of tests necessary to find
k* - 1, which in turn is the same as the number of bricks needed to find k*.
...
Test(i, j) = min(1 + max(Test(i - k, j), Test(k - 1, j - 1))) for all k in [1, i]

There will be many cases of overlapping sub-problems that would require recalculation if not cached, so top-down will be
an easy way to tackle the problem, as cases will be solved when they are first encountered. Top-down facilitates a more
intuitive implementation of the solution.
...
def test_levels(n: int, m: int) -> (list[list[int]], list[list[int]]):
    ...
    # Take care of the binary search case
    if (n - 1).bit_length() <= m:
        tests[m][n - 1] = (n - 1).bit_length()
        next_level[m][n - 1] = n // 2
    else:
        # Otherwise, make the recursive call to helper and get result
